...
Test(test_state, test_peer_exhange) {
  ...
  cr_assert(eq(int,
               sizeof(peer_message) + 4 * sizeof(peer_info) -
                   sizeof(message->message_size),
  ...
  cr_assert(
      eq(int, 1, port_1 == 1 || port_2 == 1 || port_3 == 1 || port_4 == 1));
  cr_assert(
      eq(int, 1, port_1 == 2 || port_2 == 2 || port_3 == 2 || port_4 == 2));
  cr_assert(
      eq(int, 1, port_1 == 3 || port_2 == 3 || port_3 == 3 || port_4 == 3));
  cr_assert(
      eq(int, 1, port_1 == 4 || port_2 == 4 || port_3 == 4 || port_4 == 4));
...
Test(test_state, test_broadcast_want_1piece1client) {
  ...
  int fd_in = open("/tmp/test_broadcast_want", O_CREAT | O_APPEND | O_RDWR,
                   S_IRUSR | S_IWUSR);
  if (fd_in < 0) {
    puts("failed to open test_state fd");
  }
  ...
  // message_size field
  cr_assert(
      eq(int, message.message_size, sizeof(ask_message) - sizeof(uint32_t)));
  // type field
