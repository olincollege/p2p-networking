...
Test(test_socket, check_expand_socket_buffer) {
    ...
    // only run this test on CI because of annoyance of using sudo on tests locally
    if(getenv("CI")) {
        ...
        getsockopt(sock, SOL_SOCKET, SO_RCVBUF,(void *)&original_rec_buffer, &int_size);
        ...
        getsockopt(sock, SOL_SOCKET, SO_RCVBUF,(void *)&changed_rec_buffer, &int_size);
        getsockopt(sock, SOL_SOCKET, SO_SNDBUF,(void *)&changed_send_buffer, &int_size);
